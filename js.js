var get_json = function (value, evt_name) {
  ...
  var round_value = function (evt) {  // round value
    return before_round = Math.round(evt);
  };

  var calculate_date = function (evt) { // function for calculate human readable date
    var my_date = new Date(evt * 1000);
    return my_date.toGMTString().slice(17, 22); // calculate redable date
  };

  var to_upper_case = function (evt) { // up to first letter description
  ...
  var lost_deg = function (evt) { // if JASON package lost data (direction of wind), change string.
    if (evt === undefined) {
  ...
  if (evt_name === 'today') {
    ...
    xhr.open('GET', `https://api.openweathermap.org/data/2.5/weather?q=${value}&lang=pl&units=metric&APPID=27934ae1bf6fc9febb5efe59f95f74f6`);
    xhr.send(null);
    xhr.addEventListener('load', function () {
      ...
      if (xhr.status === 200) {
        ...
        newContent += '<h1>' + response_object.name + ' - ' + response_object.sys.country + ' | ' + 'godzina:'
          + ' ' + calculate_date(time) + '</h1>';
        var val_temperature = response_object.main.temp; // temperature
        newContent += '<p>' + '<b>' + 'Temperatura:' + '</b>' + ' ' + round_value(val_temperature) + '&#8451' + '</p>';
        var val_pressure = response_object.main.pressure;  // under pressure
        newContent += '<p>' + '<b>' + 'Ciśnienie:' + '</b>' + ' ' + round_value(val_pressure) + ' ' + 'hPa' + '</p>';
        newContent += '<p>' + '<b>' + 'Wilgotność:' + '</b>' + ' ' + response_object.main.humidity + ' ' + '%' + '</p>';
        var val_description = response_object.weather[0].description;  // description
        newContent += '<p>' + '<b>' + 'Warunki:' + '</b>' + ' ' + to_upper_case(val_description) + '</p>';
        var wind_speed = response_object.wind.speed; // wind 
        var wind_degrees = response_object.wind.deg; // degrees 
        newContent += '<p>' + '<b>' + lost_deg(wind_degrees) + '</b>' + ' ' + round_value(wind_speed) + ' m/s'  + ' | ' +
          deg_to_compass(wind_degrees) + '</p>';
        newContent += '</div>';

        document.getElementById('weather_today').innerHTML = newContent;
      } else {
        var error = '<p>' + 'Jesteś pewny że istnieje takie miasto? Sprawdź jeszcze raz.' + '</p>';
    ...
    xhr.open('GET', `https://api.openweathermap.org/data/2.5/forecast?q=${value}&lang=pl&units=metric&APPID=27934ae1bf6fc9febb5efe59f95f74f6`);
    xhr.send(null);
    xhr.addEventListener('load', function () {
      ...
      if (xhr.status === 200) {
        ...
        for (var i = 0; i < response_object.list.length; i++) {
          ...
          if (i === 0) {
            newContent += '<h1>' + response_object.city.name + ' - ' + response_object.city.country + ' | ' + 'godzina:' + ' ' + 
              time.slice(11, 16) + '</h1>';
          } else {  
            var result = day_of_the_week(time); // function day_of_the_week returns object           
            create_div.className ='color' + result.color; 
            newContent += '<h1>' + '<span>' + result.day + '</span>' + ' | ' + 'godzina:' + ' ' + time.slice(11, 16) + '</h1>';
          };
          ...
          newContent += '<p>' + '<b>' + 'Ciśnienie:' + '</b>' + ' ' + round_value(val_pressure) + ' ' + 'hPa' + '</p>';
          newContent += '<p>' + '<b>' + 'Wilgotność:' + '</b>' + ' ' + response_object.list[i].main.humidity + ' ' + '%' + '</p>';
          ...
          newContent += '<p>' + '<b>' + 'Prędkość wiatru | kierunek:' + '</b>' + ' ' + round_value(wind_speed) + ' m/s' + ' | ' +
            deg_to_compass(wind_degrees) + '</p>';

          document.querySelector('#weather_five div:last-child').innerHTML = newContent;
        };
      } else {
        var error = '<p>' + 'Jesteś pewny że istnieje takie miasto? Sprawdź jeszcze raz.' + '</p>';
